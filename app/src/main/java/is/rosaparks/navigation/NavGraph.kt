...
import `is`.rosaparks.ui.screens.BackendStatusScreen
import `is`.rosaparks.ui.screens.BlikkTestScreen
...
object Routes {
    ...
    const val BACKEND_STATUS_FIRST = "backend_status_first"
    const val BLIKK_TEST = "blikk_test"
...
fun RosaParksNavGraph(viewModel: ParkingViewModel = viewModel()) {
    ...
    val startDestination =
        remember {
            if (!viewModel.backendStatusAcknowledged.value) Routes.BACKEND_STATUS_FIRST else mainStart
        }
    ...
    NavHost(navController = navController, startDestination = startDestination) {
        ...
        composable(Routes.ABOUT) {
            ...
            AboutScreen(
                ...
                onBackendStatus = {
                    navController.navigate(Routes.BACKEND_STATUS) {
                        launchSingleTop = true
                    }
                },
                onBlikkTest = {
                    navController.navigate(Routes.BLIKK_TEST) {
                        launchSingleTop = true
                    }
                },
            )
        ...
        composable(Routes.BACKEND_STATUS_FIRST) {
            BackendStatusScreen(
                firstLaunch = true,
                onAcknowledge = {
                    viewModel.acknowledgeBackendStatus()
                    navController.navigate(mainStart) {
                        popUpTo(0) { inclusive = true }
                        launchSingleTop = true
                    }
                },
                onClose = { /* not used in firstLaunch mode */ },
            )
        }
        composable(Routes.BACKEND_STATUS) {
            BackendStatusScreen(
                firstLaunch = false,
                onAcknowledge = { /* already acked */ },
                onClose = { navController.popBackStack() },
            )
        }
        // Blikk rail probe — debug builds only, so the route does not exist at
        // all in a shipped app. Reached from the debug entry on AboutScreen.
        if (BuildConfig.DEBUG) {
            composable(Routes.BLIKK_TEST) {
                BlikkTestScreen(onBack = { navController.popBackStack() })
            }
        }
    }
}
